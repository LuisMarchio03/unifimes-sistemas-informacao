...
export default function HomePage() {
  ...
  const handleLogout = () => {
    // Usar a função de logout do auth store
    logout()
  ...
  const canCreateProject = user?.role === "admin" || user?.role === "professor"
  ...
  return (
    <AppLayout title={`Olá, ${user.name || 'Usuário'}`} showSearch={true} onSearchChange={setSearchTerm} searchValue={searchTerm}>
      <div className="space-y-6">
          ...
          <div className="relative rounded-lg border border-white/10 bg-black/80 p-6">
            ...
            <div className="flex gap-3">
              <Button variant="default" size="sm">
                <Link href="/agenda" className="flex items-center justify-center gap-1">
                  <Calendar className="mr-2 h-4 w-4" />
                  Ver agenda
                </Link>
              </Button>
              <Button variant="outline" size="sm">
                <Link href="/metas" className="flex items-center justify-center gap-1">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Button variant="outline" className="h-16 flex-col gap-2 relative overflow-hidden group">
              {/* <Link href="/equipes" className="flex flex-col items-center justify-center h-full w-full z-10"> */}
        ...
        {user.role === "student" && (
          <ProjectOpportunities 
            ...
            isLoading={isLoading} 
            onApply={(projectId) => router.push(`/projetos?apply=${projectId}`)}
            limit={2}
          />
        )}
